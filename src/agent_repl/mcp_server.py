...
@mcp.tool()
async def python_repl_create() -> dict[str, Any]:
    ...
    try:
        ...
        port = await asyncio.to_thread(manager.start_repl)
...
@mcp.tool()
async def python_repl_exec(port: int, code: str) -> dict[str, Any]:
    ...
    try:
        ...
        response = await asyncio.to_thread(client.execute, port, code)
...
@mcp.tool()
async def python_repl_show(port: int) -> dict[str, Any]:
    ...
    """
    try:
        namespace = await asyncio.to_thread(client.show, port)
...
@mcp.tool()
async def python_repl_reset(port: int) -> dict[str, Any]:
    ...
    """
    try:
        success = await asyncio.to_thread(client.reset, port)
...
@mcp.tool()
async def python_repl_destroy(port: int) -> dict[str, Any]:
    ...
    """
    try:
        success = await asyncio.to_thread(client.quit_server, port)
...
@mcp.tool()
async def python_repl_check(port: int) -> dict[str, Any]:
    ...
    """
    try:
        alive = await asyncio.to_thread(manager.is_repl_alive, port)
